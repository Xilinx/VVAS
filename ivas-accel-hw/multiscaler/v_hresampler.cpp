			...
			for (k = 0; k < HSC_SAMPLES_PER_CLOCK; ++k)
			{
				...
				else
				{
					...
					if (odd_col) // odd cols (1, 3, 5, etc)
					{
						...
						outpix.val[k*HSC_NR_COMPONENTS + 1] = (pixbuf_cb[2 + (k / 2)].val[0] + pixbuf_cb[1 + (k / 2)].val[0] + 1) / 2;
						outpix.val[k*HSC_NR_COMPONENTS + 2] = (pixbuf_cr[2 + (k / 2)].val[0] + pixbuf_cr[1 + (k / 2)].val[0] + 1) / 2;
					}

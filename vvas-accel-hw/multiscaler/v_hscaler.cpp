...
static void hscale_core_polyphase(HSC_STREAM_MULTIPIX &SrcImg, U16 Height, U16 WidthIn,
		U16 WidthOut, U32 PixelRate, U8 ColorMode,
		I16 hfltCoeff[HSC_PHASES][HSC_TAPS][HSC_SAMPLES_PER_CLOCK],
		HSC_PHASE_CTRL phasesH[HSC_MAX_WIDTH / HSC_SAMPLES_PER_CLOCK], HSC_STREAM_MULTIPIX &OutImg);

static void hscale_polyphase(I16 hfltCoeff[HSC_PHASES][HSC_TAPS][HSC_SAMPLES_PER_CLOCK],
		YUV_PIXEL PixArray[HSC_ARRAY_SIZE], U16 PhasesH[HSC_SAMPLES_PER_CLOCK],
		U8 ArrayIdx[HSC_SAMPLES_PER_CLOCK], YUV_MULTI_PIXEL *OutPix);
#endif
		...
		HSC_STREAM_MULTIPIX &stream_out)
{
	if (bPassThruHsc)
	{
		...
		for (y = 0; y < (Height); ++y)
		{
			for (x = 0; x < (WidthIn) / HSC_SAMPLES_PER_CLOCK; ++x)
			{
...
#pragma HLS PIPELINE
	...
	else
	{
		...
		{
			hscale_core_polyphase(stream_in, Height, WidthIn, WidthOut, PixelRate, ColorMode,
					hfltCoeff, PhasesH, stream_out);
		}
#else
		{
 ...
 *               hscale_poly
 **********************************************************************************/
static void hscale_core_polyphase(HSC_STREAM_MULTIPIX &SrcImg, U16 Height, U16 InPixels,
		U16 OutPixels, U32 Rate, U8 RegColorMode,
		I16 hfltCoeff[HSC_PHASES][HSC_TAPS][HSC_SAMPLES_PER_CLOCK],
		HSC_PHASE_CTRL arrPhasesH[HSC_MAX_WIDTH / HSC_SAMPLES_PER_CLOCK],
		HSC_STREAM_MULTIPIX &OutImg)
{
	...
	LoopSize = ((LoopSize + (HSC_SAMPLES_PER_CLOCK - 1)) / HSC_SAMPLES_PER_CLOCK)
...
#pragma HLS pipeline II=1
			...
			if (ReadEn) // If new pixels to be got, shift the old ones
			{
				...
				for (int i = 0; i <= (HSC_ARRAY_SIZE - 1 - HSC_SAMPLES_PER_CLOCK); i++)
				{
					...
					PixArray[i] =
							(((x / HSC_SAMPLES_PER_CLOCK) <= 1)
									&& i < (HSC_ARRAY_SIZE - 2 * HSC_SAMPLES_PER_CLOCK)) ?
									PixArray[HSC_ARRAY_SIZE - 1 - HSC_SAMPLES_PER_CLOCK + 1] :
									PixArray[i + HSC_SAMPLES_PER_CLOCK];
				}
				// Read new pixels if we didn't exceed the width of frame yet
				if ((xReadPos + HSC_ARRAY_SIZE - 1 - (HSC_TAPS >> 1)) < InPixels)
				{
					...
					for (int i = 0; i < HSC_SAMPLES_PER_CLOCK; i++)
					{
						for (int k = 0; k < HSC_NR_COMPONENTS; k++)
						{
							PixArray[HSC_ARRAY_SIZE - 1 - HSC_SAMPLES_PER_CLOCK + 1 + i].val[k] =
									SrcPix.val[HSC_NR_COMPONENTS * i + k];
						}
					}
				}
				else
				{
					...
					for (int i = 0; i < HSC_SAMPLES_PER_CLOCK; i++)
					{
						PixArray[HSC_ARRAY_SIZE - 1 - HSC_SAMPLES_PER_CLOCK + 1 + i] =
								PixArray[HSC_ARRAY_SIZE - 1 - HSC_SAMPLES_PER_CLOCK];
					}
...
loop_samples:
				for (int s = 0; s < HSC_SAMPLES_PER_CLOCK; s++)
				{
					...
					//
					/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
					PhaseH[s] = phases(HSC_PHASE_CTRL_PHASE_MSB + s * HSC_PHASE_CTRL_BITS,
							HSC_PHASE_CTRL_PHASE_LSB + s * HSC_PHASE_CTRL_BITS);
					ArrayLoc[s] = phases(HSC_PHASE_CTRL_INDEX_MSB + s * HSC_PHASE_CTRL_BITS,
							HSC_PHASE_CTRL_INDEX_LSB + s * HSC_PHASE_CTRL_BITS);
					OutputWriteEn[s] = phases[HSC_PHASE_CTRL_ENABLE_LSB + s * HSC_PHASE_CTRL_BITS];
				}
				ReadEn = (ArrayLoc[HSC_SAMPLES_PER_CLOCK - 1] >= HSC_SAMPLES_PER_CLOCK);

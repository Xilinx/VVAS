...
static void vscale_core_polyphase(VSC_STREAM_MULTIPIX &SrcImg, U16 HeightIn, U16 WidthIn,
		U16 HeightOut, U32 LineRate, I16 vfltCoeff[VSC_PHASES][VSC_TAPS],
		VSC_STREAM_MULTIPIX &OutImg);

static void vscale_polyphase(I16 vfltCoeff[VSC_PHASES][VSC_TAPS],
		YUV_MULTI_PIXEL PixArray[VSC_TAPS], U8 PhaseV, YUV_MULTI_PIXEL *OutPix);
 ...
 **********************************************************************************/
void v_vscaler(VSC_STREAM_MULTIPIX &stream_in, U16 HeightIn, U16 WidthIn, U16 HeightOut,
		...
		VSC_STREAM_MULTIPIX &stream_out)
{
	if (bPassThruVsc)
	{
		...
		assert(HeightIn <= HSC_MAX_HEIGHT);
...
#else
		{
			vscale_core_polyphase(stream_in, HeightIn, WidthIn, HeightOut, LineRate, vfltCoeff,
					stream_out);
		}
#endif
	}
			...
			for (I16 i = 0; i < VSC_TAPS; i++)// get cols of pixels from the line buffer to local pixel array
			{
				...
				for (int l = 0; l < VSC_SAMPLES_PER_CLOCK; l++)
				{
					for (int k = 0; k < VSC_NR_COMPONENTS; k++)
					{
						...
						PixArray[VSC_TAPS - 1 - i].val[k + l * VSC_NR_COMPONENTS] = LineBufVal(
								start + VSC_BITS_PER_COMPONENT - 1, start);
					}
				}
			}
			...
			if ((GetNewLine == 1) || (y <= (VSC_TAPS >> 1))) // If new pixels to be filled in the line buffer
			{
				...
				if ((PixArrayLoc + (VSC_TAPS >> 1)) < InLines) // get new pixels from stream
				{
					...
					for (int l = 0; l < VSC_SAMPLES_PER_CLOCK; l++)
					{
						...
						{
							int start = (k + l * VSC_NR_COMPONENTS) * VSC_BITS_PER_COMPONENT;
							InPix(start + VSC_BITS_PER_COMPONENT - 1, start) = SrcPix.val[k
									+ l * VSC_NR_COMPONENTS];
						}
					}
				...
				for (int i = (VSC_TAPS - 1); i > 0; i--) // for circular buffer implementation
				{
					...
					YUV_MULTI_PIXEL PixArrayVal =
							(y > 0) ? PixArray[VSC_TAPS - 1 - i] : PixArray[VSC_TAPS - 1];
					for (int l = 0; l < VSC_SAMPLES_PER_CLOCK; l++)
					{
						for (int k = 0; k < VSC_NR_COMPONENTS; k++)
						{
							int start = (k + l * VSC_NR_COMPONENTS) * VSC_BITS_PER_COMPONENT;
							LineBufVal(start + VSC_BITS_PER_COMPONENT - 1, start) =
									PixArrayVal.val[k + l * VSC_NR_COMPONENTS];
						}
					}

		...
		bool bPassThru, HSC_STREAM_MULTIPIX &outImg)
{
	...
	const U8 PIXBUF_C_DEPTH = (((((((KERNEL_H_SIZE / 2) + 1) + HSC_SAMPLES_PER_CLOCK - 1)
			+ (HSC_SAMPLES_PER_CLOCK - 1)) / HSC_SAMPLES_PER_CLOCK) * HSC_SAMPLES_PER_CLOCK)
			+ (KERNEL_H_SIZE / 2) - 1);
...
#pragma HLS PIPELINE II=1
			...
			if (x < (width / HSC_SAMPLES_PER_CLOCK))
			{
				...
				{
					mpix_y.val[s] = inpix.val[s * HSC_NR_COMPONENTS];
					if (inColorMode == yuv444)
					{
						mpix_cb.val[s] = inpix.val[s * HSC_NR_COMPONENTS + 1];
						mpix_cr.val[s] = inpix.val[s * HSC_NR_COMPONENTS + 2];
					}
					else
					{
						if (((x * HSC_SAMPLES_PER_CLOCK) + s) & 1)
						{
							mpix_cr.val[s / 2] = inpix.val[s * HSC_NR_COMPONENTS + 1];
						}
						else
						{
							mpix_cb.val[s / 2] = inpix.val[s * HSC_NR_COMPONENTS + 1];
						}
					}
				}
			}
...
#pragma HLS ARRAY_PARTITION variable=pixbuf_y complete dim=0
			...
			for (k = 0; k < HSC_SAMPLES_PER_CLOCK; k++)
			{
				...
				if (CRpix == 1) // work with Cb/Cr pairs
				{
					...
					{
						pixbuf_cb[PIXBUF_C_DEPTH - 1].val[0] = mpix_cb.val[(HSC_SAMPLES_PER_CLOCK
								/ shift) - 1];
						pixbuf_cr[PIXBUF_C_DEPTH - 1].val[0] = mpix_cr.val[(HSC_SAMPLES_PER_CLOCK
								/ shift) - 1];
					}
				}
				//left edge padding
				if (first_pix == 1)
				{
					...
					{
						for (I16 i = (PIXBUF_C_DEPTH - (SAMPLES_PER_CLOCK / 2));
								i > (PIXBUF_C_DEPTH - SAMPLES_PER_CLOCK); --i)
						{
			...
			for (k = 0; k < HSC_SAMPLES_PER_CLOCK; ++k)
			{
				...
				if (inColorMode == yuv444)
				{
					...
					// luma component
					outpix.val[k * HSC_NR_COMPONENTS] = pixbuf_y[center_tap + k].val[0];
					if (chroma_out_pix == 1)
					{
						// filter
						filt_res0 = (pixbuf_cb[0 + ((k / 2) * 2)].val[0]
								+ 2 * pixbuf_cb[1 + ((k / 2) * 2)].val[0]
								+ pixbuf_cb[2 + ((k / 2) * 2)].val[0] + 2) / 4;
						filt_res1 = (pixbuf_cr[0 + ((k / 2) * 2)].val[0]
								+ 2 * pixbuf_cr[1 + ((k / 2) * 2)].val[0]
								+ pixbuf_cr[2 + ((k / 2) * 2)].val[0] + 2) / 4;
					}
					outpix.val[k * HSC_NR_COMPONENTS + 1] = (odd_col) ? filt_res1 : filt_res0;
					outpix.val[k * HSC_NR_COMPONENTS + 2] = 0;
				}
				else
				{
					...
					if (odd_col) // odd cols (1, 3, 5, etc)
					{
						...
						outpix.val[k * HSC_NR_COMPONENTS + 1] = (pixbuf_cb[2 + (k / 2)].val[0]
								+ pixbuf_cb[1 + (k / 2)].val[0] + 1) / 2;
						outpix.val[k * HSC_NR_COMPONENTS + 2] = (pixbuf_cr[2 + (k / 2)].val[0]
								+ pixbuf_cr[1 + (k / 2)].val[0] + 1) / 2;
					}
					else // even cols (0, 2, 4, etc)
					{
						// passthru co-sited pixel
						outpix.val[k * HSC_NR_COMPONENTS + 1] = pixbuf_cb[1 + (k / 2)].val[0];
						outpix.val[k * HSC_NR_COMPONENTS + 2] = pixbuf_cr[1 + (k / 2)].val[0];
					}
				}

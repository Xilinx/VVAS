...
if [ $# -ne 3 ]
  then
    ...
    echo "Usage: ./01_rotate_video.sh <device index> <1080p60 h264 clip> <direction {0..7}>"
    exit 1
fi
...
pipe="gst-launch-1.0 -v filesrc location=$INPUT ! h264parse ! vvas_xvcudec xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin  dev-idx=$di ! video/x-raw, format=NV12 ! videoflip video-direction=${DIRECTION} ! vvas_xvcuenc xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin  dev-idx=$di  target-bitrate=8000 ! h264parse ! qtmux ! fpsdisplaysink video-sink=\"filesink location=/tmp/xil_rotate_${DIRECTION}.mp4\" sync=false text-overlay=false"

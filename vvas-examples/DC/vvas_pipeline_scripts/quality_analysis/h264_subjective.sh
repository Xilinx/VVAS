...
pipe="filesrc location=$2 blocksize=3110400 ! video/x-raw, width=1920, height=1080, format=NV12, framerate=60/1 ! vvas_xlookahead xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin  codec-type=0 lookahead-depth=20 spatial-aq=true temporal-aq=true dev-idx=$di b-frames=1 ! vvas_xvcuenc xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin dev-idx=$di b-frames=1 gop-length=120 periodicity-idr=120 qp-mode=relative-load rc-mode=1 target-bitrate=$3 max-bitrate=$3 ! video/x-h264 !  fpsdisplaysink video-sink=\"filesink location=/tmp/xil_enc_${bitrate}_subjective.h264 \" text-overlay=false sync=false "$pipe

...
pipe="filesrc location=$2 blocksize=3110400 ! video/x-raw, width=1920, height=1080, format=NV12, framerate=60/1 ! vvas_xvcuenc xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin dev-idx=$di  b-frames=0 gop-length=120 periodicity-idr=120 qp-mode=auto target-bitrate=$3 max-bitrate=$3 ! video/x-h265 ! fpsdisplaysink video-sink=\"filesink location=/tmp/xil_enc_${bitrate}_ll_subjective.h265 \" text-overlay=false sync=false "$pipe

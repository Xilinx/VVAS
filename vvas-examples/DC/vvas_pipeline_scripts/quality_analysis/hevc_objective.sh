...
pipe="filesrc location=$2 blocksize=3110400 ! video/x-raw, width=1920, height=1080, format=NV12, framerate=60/1 ! vvas_xlookahead xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin codec-type=1 lookahead-depth=20 spatial-aq=0 temporal-aq=0 dev-idx=$di ! vvas_xvcuenc xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin dev-idx=$di  b-frames=2 gop-length=120 periodicity-idr=120 qp-mode=uniform scaling-list=flat rc-mode=1 target-bitrate=$3 max-bitrate=$3 ! video/x-h265 ! fpsdisplaysink video-sink=\"filesink location=/tmp/xil_enc_${bitrate}_objective.h265 \" text-overlay=false sync=false "$pipe

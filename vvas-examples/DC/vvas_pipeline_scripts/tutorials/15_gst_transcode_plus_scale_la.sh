...
if [ $# -lt 3 ]; then
     ...
     echo ""	
     echo -e "\e[01;31m"e.g $0 0 bbb_sunflower_1080p_60fps_normal.mp4 2 2000 0"\e[0m"
     echo "<Number of buffers> - Input number of buffers to be processed from input file (each buffer is of size 4K)" 
     echo "NOTE: Running this script displays the average fps values on terminal for all instances using notation <reoslution>_<deviceid>_<instance_number>"
     ...
     echo "This script runs all the instances in the background and can not be killed using ctrl+C"
...
do
	sc_name="sc_"$c$d
        ...
        fps_sink_720p60="sink_xcode_scale_720p60_dev"$d"_"$c
        fps_sink_720p30="sink_xcode_scale_720p30_dev"$d"_"$c
        fps_sink_480p30="sink_xcode_scale_480p30_dev"$d"_"$c
        fps_sink_360p30="sink_xcode_scale_360p30_dev"$d"_"$c
        fps_sink_160p30="sink_xcode_scale_160p30_dev"$d"_"$c

if [[ $5  -eq 0 ]]; then
        echo "Script is running for filesink...."
	muxer="! qtmux"
        sink_720p60="\"filesink location=/\tmp/\xil_la_xcode_scale_720p60_dev_${Depth}_${d}_${c}.mp4 async=false\""
        sink_720p30="\"filesink location=/\tmp/\xil_la_xcode_scale_720p30_dev_${Depth}_${d}_${c}.mp4 async=false\""
        sink_480p30="\"filesink location=/\tmp/\xil_la_xcode_scale_480p30_dev_${Depth}_${d}_${c}.mp4 async=false\""
        sink_360p30="\"filesink location=/\tmp/\xil_la_xcode_scale_360p30_dev_${Depth}_${d}_${c}.mp4 async=false\""
        sink_160p30="\"filesink location=/\tmp/\xil_la_xcode_scale_160p30_dev_${Depth}_${d}_${c}.mp4 async=false\""
else
        echo "Script is running for fakesink...."
	muxer=
        sink_720p60="\"fakesink async=false\""
        sink_720p30="\"fakesink async=false\""
        sink_480p30="\"fakesink async=false\""
        sink_360p30="\"fakesink async=false\""
        sink_160p30="\"fakesink async=false\""
fi
...
pipe="gst-launch-1.0 filesrc num-buffers=$num location=$input_file
	...
	! vvas_xabrscaler  xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin avoid-output-copy=true  dev-idx=$di ppc=4 scale-mode=2 name=$sc_name
	$sc_name.src_0
	    ! queue
		...
		! vvas_xlookahead  xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin  codec-type=0 spatial-aq=1 temporal-aq=1 lookahead-depth=8 dev-idx=$di
		! vvas_xvcuenc xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin name=$enc_720p60 dev-idx=$di  target-bitrate=4000 rc-mode=1
		! h264parse
		${muxer}
		...
		! vvas_xlookahead  xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin  codec-type=0 spatial-aq=1 temporal-aq=1 lookahead-depth=8 dev-idx=$di
		! vvas_xvcuenc xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin name=$enc_720p30 dev-idx=$di  target-bitrate=3000 rc-mode=1
		! h264parse
		${muxer}
	...
	$sc_name.src_1
	    ...
	    ! video/x-raw, framerate=30/1
	    ! vvas_xlookahead  xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin  codec-type=0 spatial-aq=1 temporal-aq=1 lookahead-depth=8 dev-idx=$di
	    ! vvas_xvcuenc xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin name=$enc_480p30 dev-idx=$di  target-bitrate=2500 rc-mode=1
	    ! h264parse
	    ${muxer}
	...
	$sc_name.src_2
	    ...
	    ! video/x-raw, framerate=30/1
	    ! vvas_xlookahead  xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin  codec-type=0 spatial-aq=1 temporal-aq=1 lookahead-depth=8 dev-idx=$di
	    ! vvas_xvcuenc xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin name=$enc_360p30 dev-idx=$di  target-bitrate=1250 rc-mode=1
	    ! h264parse
	    ${muxer}
	...
	$sc_name.src_3
	    ...
	    ! video/x-raw, framerate=30/1
	    ! vvas_xlookahead  xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin  codec-type=0 spatial-aq=1 temporal-aq=1 lookahead-depth=8 dev-idx=$di
	    ! vvas_xvcuenc xclbin-location=/opt/xilinx/xcdr/xclbins/transcode.xclbin name=$enc_160p30 dev-idx=$di  target-bitrate=625 rc-mode=1
	    ! h264parse
	    ${muxer}

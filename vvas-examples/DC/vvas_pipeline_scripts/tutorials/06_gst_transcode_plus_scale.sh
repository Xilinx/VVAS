...
# This script assumes an 8-bit, YUV420, pre-encoded 60FPS mp4 file with h.264 content.
...
# All outputs will be stored at /tmp/sink_xcode_scale_*.mp4
...
if [ $# -lt 3 ]; then
     ...
     echo "NOTE: Running this script writes encoded ourput files to /tmp/ direcotry and displays the average fps values on terminal for all instances using notation <reoslution>_<deviceid>_<instance_number>"
...
if [[ $5  -eq 0 ]]; then
	...
	muxer="! qtmux"
        sink_720p60="\"filesink location=/\tmp/\xil_xcode_scale_720p60_dev_${Depth}_${d}_${c}.mp4\""
        sink_720p30="\"filesink location=/\tmp/\xil_xcode_scale_720p30_dev_${Depth}_${d}_${c}.mp4\""
        sink_480p30="\"filesink location=/\tmp/\xil_xcode_scale_480p30_dev_${Depth}_${d}_${c}.mp4\""
        sink_360p30="\"filesink location=/\tmp/\xil_xcode_scale_360p30_dev_${Depth}_${d}_${c}.mp4\""
        sink_160p30="\"filesink location=/\tmp/\xil_xcode_scale_160p30_dev_${Depth}_${d}_${c}.mp4\""
else
